...
namespace PRoConEvents {
    public class AdKats : PRoConPluginAPI, IPRoConPluginInterface {
        ...
        private Dictionary<String, Func<AdKats, Double>> _commandTimeoutDictionary = new Dictionary<string, Func<AdKats, double>>(); 
        ...
        private void QueueRecordForProcessing(AdKatsRecord record) {
            ...
            try {
                //Check if the record is on timeout
                if (record.target_player != null && _commandTimeoutDictionary.ContainsKey(record.command_action.command_key))
                {
        ...
        private void SubmitBanToMetabans(AdKatsBan aBan) {
            this.DebugWrite("^4Metabans (SubmitAssessment): Submitting assessment of GUID " + aBan.ban_record.target_player.player_guid, 3);

            MetabansAPI api = new MetabansAPI(this._metabansUsername, this._metabansAPIKey, enumBoolOnOff.On);
            ...
            switch (this._gameVersion) {
                ...
                case GameVersion.BF4:
                    ...
                    break;
                default:
                    this.ConsoleError("Invalid game version when posting to metabans.");
                    return;
                    break;
            }
            api.mb_assess_player(
                gameType,
                aBan.ban_record.target_player.player_guid,
                AssessmentTypes.black, 
                aBan.ban_record.record_message,
                (int)(aBan.ban_endTime - aBan.ban_startTime).TotalSeconds
            ).Post();
        }
        ...
        public void api_mb_assess_player_ok(Hashtable request, Hashtable data)
        {
            try
            {
                this.ConsoleWarn("^4Metabans (api_mb_assess_player_ok): Assessment accepted");
            }
            catch (Exception e)
            {
                this.ConsoleWarn("^1Metabans (api_mb_assess_player_ok): " + e.Message);
            }
        }
        ...
        private void PushThreadDebug(Int64 ticks, String thread, Int32 threadid, Int32 line, String element) {
            try {
                DebugWrite(ticks + " " + thread + " " + threadid + " " + line + " " + element, 7);
            }
            catch (Exception e) {
...
<br><a href='http://metabans.com/search/%player_name%'>Metabans Search</a><br>";
        ...
        //Below is the current Metabans API version 1.0.0.4
        //Author: Phouge
